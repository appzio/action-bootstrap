...
class MobileexampleAccessor extends ArticleModel {


    public static function getList($playid,$keywordsearch,$keywordvariable,$limit=false){
        $keywordsearch = 'a';
        $array = self::getLetter($keywordsearch,$limit);
        return $array;
    }
    ...
    public static function getLetter($key,$loosematch=false,$limit=999){
        $words = self::getWords();
        $output = array();
        $key = strtolower($key);
